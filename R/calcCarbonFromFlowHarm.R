...
#' @examples
#' \dontrun{
#' calcOutput("calcCarbonFromFlowHarm", aggregate = FALSE)
#' }
#'
#' @importFrom magpiesets findset
#' @importFrom magclass add_dimension

calcCarbonFromFlowHarm <- function(lpjml = c(natveg = "LPJmL4_for_MAgPIE_44ac93de",
                                             crop = "ggcmi_phase3_nchecks_9ca735cb"),
                                   climatetype = "GSWP3-W5E5:historical") {

  # Create settings for LPJmL from version and climatetype argument
  cfgLPJmL <- toolLPJmLVersion(version = lpjml["natveg"], climatetype = climatetype)
  ...
  cfg         <- list(lpjml = lpjml["natveg"], climatetype = cfgLPJmL$baseline_hist)
