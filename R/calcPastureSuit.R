...
calcPastureSuit <- function(subtype = "ISIMIP3b:IPSL-CM6A-LR:1850-2100", smooth_precipitation = 10, smooth_out = 10) {
  ...
  # pasture drivers
  population <- calcOutput("GridPop_new", subtype = "all", cellular = TRUE, FiveYear = TRUE, harmonize_until = 2015, aggregate = FALSE)
  ...
  for (scenario in scenarios) {
    ...
    precipitation[[scenario]] <- setNames(calcOutput("GCMClimate_new", subtype = subtype, smooth = smooth_precipitation, aggregate = FALSE), paste0("past_suit.", scenario))
  }
  precipitation <- mbind(precipitation)

  # matching available ssps scenarios
  regex <- paste0("[", paste0("+", strtrim(scenarios, 4), collapse = "|"), "]", "{4}$")
  avl_ssps <- grep(regex, getNames(population), ignore.case = TRUE)
  ...
  hist_pastr <- calcOutput("LUH2v2", aggregate = FALSE, landuse_types = "LUH2v2", cellular = TRUE)[, , "pastr"]
