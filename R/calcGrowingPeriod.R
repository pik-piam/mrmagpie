...
calcGrowingPeriod <- function(lpjml=c(natveg="LPJmL4_for_MAgPIE_84a69edd", crop="ggcmi_phase3_nchecks_72c185fa"),
  ...
  if(stage%in%c("raw","smoothed")){
    ...
    sowd         <- toolCoord2Isocell(collapseNames(calcOutput("LPJmL_new", version=lpjml["crop"], climatetype=climatetype,
                                             subtype="sdate", stage="raw", aggregate=FALSE)[,,"rainfed"]))
    hard         <- toolCoord2Isocell(collapseNames(calcOutput("LPJmL_new", version=lpjml["crop"], climatetype=climatetype,
                                             subtype="hdate", stage="raw", aggregate=FALSE)[,,"rainfed"]))
