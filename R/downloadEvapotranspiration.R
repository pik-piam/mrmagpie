...
downloadEvapotranspiration <- function(subtype="H08:mri-esm2-0:historical") {
  ...
  storage     <- "/p/projects/isimip/isimip/ISIMIP3b/OutputData/water_global"

  if (x$scenario == "historical") {
    path        <- file.path(storage, x$water_model, x$climate_model, "historical")
  } else {
    path        <- file.path(storage, x$water_model, x$climate_model,"future")
  }
