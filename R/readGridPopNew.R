...
readGridPopNew <- function(subtype) {
  ...
  if (subtype == "past") {
    ...
    years     <- as.numeric(unlist(regmatches(pastFile, gregexpr("\\d{4}", pastFile))))
  ...
  if (subtype == "future") {
    ...
    sspNaming <- NULL
    for (file in files) {
      sspNaming <- append(sspNaming, paste0("pop_", toupper(unlist(regmatches(file, gregexpr("ssp[0-9]", file))))))
      x[[file]] <- read(file)
    }
    x <- mbind(x)
