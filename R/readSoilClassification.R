...
readSoilClassification <- function(subtype = "HWSD.soil") {
    ...
    if (dir.exists(folder)) {
      ...
      files <- c(soil = filesList[grep("soil", filesList)])
    } else {
      stop("Path ", folder, " does not exist. Check the defition of your ",
           "subtype or the folder structure you are trying to access.")
    }

    fileName <- toolSubtypeSelect(subtype, files)

    sk <- file(file.path(folder, fileName), "rb")
